...
export const getSubCateg=(request,response) => {
    ...
    let {ctg} = request.params
    if(ctg==0){
        sql2 = "SELECT subcategory.description FROM subcategory ORDER BY description"
    }
    else{
        sql2 = "SELECT subcategory.description FROM subcategory,category WHERE subcategory.id=category.subcateg_id AND subcategory.id=?"
    }
